...
| **Config-integrity** — connector config (datastreamId / edgeConfigId) host-owned, not chamber-mutable — **URL-surface enforcement DELIVERED** ([spec 015-01](../specs/015-mvp3-config-integrity-enforcement/slice-01-fail-closed-enforcement.md) DONE 2026-08-30; [ADR-0011](../decisions/adr-0011-config-integrity-enforcement.md)): generic host+injected-tenant-key control in `core/wrapped-sdk-host.js`'s dispatch seam, **fail-closed HOLD** + redacted 009-02 alert on any deviation (foreign host / tenant absent / pollution / mismatch). **Residual:** the body-`orgId` co-vector is out of the URL check surface (uncovered + silent) — a tracked live-probe follow-up ([refinement-todo](../refinement-todo.md)). | Same-host-tenant re-routing defense (013-03, confirmed live) | The endpoint ceiling is blind to same-host re-routing; now recorded in ADR-0011 (ADR-0006 stays tenant-blind) |
