...
| ↳ **022-01 DELIVERED** ([spec 022](../specs/022-helix-rum-connector/spec.md), slice-01 2026-09-01): the `helix-rum` connector (`connectors/helix-rum/`) reproduces the AEM RUM `top`/page-view beacon **natively** (mechanism B, grounded — the enhancer's mechanism-A hosting is NOT cleanly feasible, see the slice's Findings) as `airlock/helix-rum`, hosted via the SAME `core/connector-host.js` GA4/alloy use. Confined to `ot.aem.live` (endpoint ceiling, exact `.rum/${weight}` match) and **NOT consent-gated** (`purposes.egress: []`, free on the existing `core/airlock.js` seam) — a payload-hygiene-by-construction 5-field beacon. | 022-01 | |
| ↳ **022-02 DELIVERED** (slice-02, 2026-09-01): widens the connector to the **`error` checkpoints** (`events: ["top","error"]`) — a 7-field beacon (5 base + whitelisted `source`/`target`, matching `sampleRUM`'s `dataFromErrorObj`) riding the SAME confined, not-consent-gated path — **and** full **sampling-rate fidelity** (`on`/`high`/`medium`/`low`/`off` rate-name → `1`/`10`/`100`/`1000`/`0` weight resolution, mirroring `aem.js`'s own table; a raw `weight` number still wins if given). `id`/`weight` stay ONE per-page identity shared across `top` + `error`. Only the CWV/interaction checkpoints (022-04, a new runtime capture) and the page-side cutover (022-03) remain. | 022-02 | |
| ↳ **022-04 DELIVERED** (slice-04, 2026-09-01): widens the connector to the **`cwv` checkpoint** (`events: ["top","error","cwv"]`) — LCP/CLS/INP via a NEW main-thread capture module (`connectors/helix-rum/cwv-capture.js`, Google's `web-vitals/attribution@6.2.1`, `onLCP`/`onCLS`/`onINP`), NOT the hosted enhancer (022-01's grounding: the enhancer can't host in a chamber). Fixes the frame-critique's structured-clone must-fix: `projectCwv` projects each metric's `attribution` sub-object to structured-cloneable SCALARS ONLY (a `typeof`-based structural filter, not a field-name list) on the MAIN THREAD before `push()`, so the non-cloneable `PerformanceEntry`/DOM-Node-carrying sub-objects (`processedEventEntries`, `longAnimationFrameEntries`, `longestScript.entry`, `largestShiftSource`, …) never reach the worker `postMessage` boundary — proven by a `structuredClone()` round-trip test fed a realistic attribution fixture including those exact hazard fields. `map.js`'s new `cwvFields` independently whitelists the same 21-field grounded superset (LCP/CLS/INP attribution scalars, `node_modules/web-vitals` `.d.ts` citations) onto the wire body — a second, wire-contract-hygiene layer. One `cwv` beacon per finalized metric (not combined — `onLCP`/`onCLS`/`onINP` finalize independently). Rides the SAME confined, not-consent-gated path + per-page `id`/`weight` as `top`/`error`. | 022-04 | |
| ↳ **RUM parked as the governance exemplar** (2026-09-01): `top`/`error`/`cwv` govern egress (confined, not-consent-gated) — the **governance** half of the thesis, proven on a real Adobe connector. **Full parity + the page-side cutover (022-05/03) are DEFERRED, NOT pursued natively** — reimplementing Adobe's evolving, plugin-extended enhancer is the wrong bet for a governance runtime; full parity comes via the future **worker-dom compat layer** or a community connector. The **performance** half (containing costly-DOM martech) moves to its own investigation ([R-008](../research/R-008-costly-dom-martech-containment.md)) + a nasty-tag POC. | R-008 | |
...
| ↳ **DELIVERED via [spec 021](../specs/021-mvp4-hardening/spec.md)** (all three slices DONE 2026-08-31): **021-01** — `dispose()` + idempotent re-boot (`createAirlock`'s handle gains `dispose()` — named unload-listener refs + `worker.terminate()`, idempotent + null-safe; `bootEdsAnalytics` disposes any prior `window.airlock` before re-boot, never stacking a second Worker/listener set; OQ12 now COMPLETE). **021-02** — the transport pin landed **in `core/config-integrity.js`** (scheme-match against the pin's `pinnedScheme`, holds an `http://` downgrade even on the ceiling-less standalone path; origin-aware, not hardcoded-https — the grounding found the more-targeted location than the ADR-0004 allow-list the original review pointed at). **021-03** — adopted ESLint (flat `eslint.config.js` on `@eslint/js` recommended + a CI `Lint` gate) and **removed** the alloy chamber's whole-file `/* eslint-disable */` outright (under `recommended` it needs none). | 021-01/02/03 | The three "must-close before real use" residuals are closed. |
