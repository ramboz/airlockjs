...
**Goal:** Adopt jig in airlock by running `scaffold-init`, so the
project opens with the jig docs tree, an honest CLAUDE.md, a populated
status board, and a runnable jig workflow — a complete pattern for the
spec-driven loop from turn one.
...
opening airlock finds a complete `DONE` worked example *and* a
