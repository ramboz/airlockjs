...
**new** runtime capture (an extension of mechanism B); the **core** beacon (022-01: page-view + errors) needs
neither, which is why it leads.
...
- **[grounded, scope-shaping] airlock has no *runtime* per-page CWV capture yet.** Its CWV measurement is
  ...
  *enhancer's* CWV checkpoints (LCP/CLS/INP) implies **building** a runtime capture — real work, isolated to
  022-02. The 022-01 core (page-view + `error` checkpoints) is main-thread-trivial and needs no CWV capture,
