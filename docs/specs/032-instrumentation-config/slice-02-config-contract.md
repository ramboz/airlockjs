...
- [x] Primer hygiene (spec 025-01): this slice closes spec 032. CLAUDE.md's Active-specs never carried a 032 entry
      (only the 001 worked example), so nothing to compress there; the config-surface invariant was migrated to the
      status-board Notes column (032-02 row), and `contracts/README.md` + `README.md` document the config surface +
      its pre-1.0 status.
