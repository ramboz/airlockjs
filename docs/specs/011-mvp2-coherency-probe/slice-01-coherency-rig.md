...
1. **Model-agnostic two-worker proxy.** A rig under `probes/coherency/` (or
   `rig/coherency.*`) stands up a main-thread **broker** holding the
   authoritative jar (the real `document.cookie`) and **two** worker chambers,
   each seeded at boot with a sync-cache of one shared identity cookie
   (`AMCV_*` / `demdex`-shaped value, per R-004) and writing back to the broker
   asynchronously. The two-worker shape is documented in the rig as an
   *instrument* for the multi-cache failure mode, explicitly not a commitment to
   ADR-0001 Option B.
