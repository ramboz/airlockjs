...
### OQ13 — Identity-cookie follow-ups (post-004-03 review residue) — item 1 RESOLVED 2026-08-30 (017-02); item 2 RESOLVED 2026-09-08 (039-03); item 4 RESOLVED 2026-09-05 (035-01) for the live grant; item 5 RESOLVED 2026-09-11 (043-01); item 3 open
...
**Resolved (slice [039-03](specs/039-ga4-gtag-connector/slice-03-session-state-writer.md), 2026-09-08):** item **2** (OQ13-2, session-cookie persistence) — ~~on a gtag-free site nothing writes `_ga_<stream>`, so an MPA mints a fresh session per page~~. `writeGa4SessionState` (`connectors/ga4/cookies.js`, host-called, `analytics_storage`-gated, reusing the 017-02 seam) is now the `_ga_<stream>` read-modify-write session writer, reproducing gtag's GS2 state machine (first-visit / same-session continuation / >timeout new session; `timeout = config.sessionTimeoutMinutes`, default 30) live-observed 2026-09-08. Sessions persist across an MPA (`sid`/`sct` reused within a session), verified field-for-field against `test/fixtures/parity-ga4-collect-multipage.redacted.json`. **Deferred to the host-wiring slice:** no production caller invokes `writeGa4SessionState` yet — the host must override `ctx.sessionId` with the writer's return and derive `streamCookieName` from the measurement id (see 039-03's deviation log).
**Resolved (slice [035-01](specs/035-cookie-grant-wrapper/slice-01-name-scoped-wrapper.md), 2026-09-05) — item 4, for the ONE live connector cookie grant (alloy's):** the frame-critique RETARGETED item 4 off `adapters/eds/cookies.js` (`createCookieCapability`) — verified HOST-side-only, granted to NO connector (its docstring: "no connector grant flow is exercised yet") — onto the real live alloy grant seams. Landed default-deny name-scope + name-validation on BOTH halves, enforced host-side on the trusted seam (034-01 — never trusting the untrusted chamber to scope itself): (a) READ — `scopeSeedCookies` (`core/cookie-scope.js`) filters the boot seed to alloy's declared names BEFORE `host.init`, closing the live whole-jar READ leak (a chamber granted `kndctr_`/`AMCV_`/`demdex` can no longer read `_ga`/session/auth cookies); (b) WRITE — the `cookie-writeback` handler (`core/wrapped-sdk-host.js`) validates the name (RFC 6265 token — `isValidCookieName`) + scopes it (`matchesGrantedName`, exact-vs-prefix) before `caps.cookies.reconcile`, dropping+diagnosing (`kind:"cookie-scope"`, name-only redaction) an invalid/un-granted name. One SSOT: `ALLOY_COOKIE_NAMES` (`connectors/alloy/connector.js`) feeds both seams. Opt-in (`grantedCookieNames` null ⇒ byte-identical to pre-035-01). The exact-vs-prefix semantic is now documented at the `CapabilityRequest.cookies` declaration site (`contracts/capability.d.ts`). **Carried follow-ons (named, NOT closed):** (i) the `{get,set}` name-scoping wrapper for `createCookieCapability` itself — still the raw whole-jar host accessor, but granted to no connector, so hardened if/when one is; (ii) the `SecurityError`→graceful-null-identity rider (still degrades to the visible `__airlockBootFailed`); ~~(iii) **arch #1 (035-01, → 037 1.0 pin) fail-open-by-omission** — a FUTURE connector wiring `caps.cookies.reconcile` but omitting `grantedCookieNames` silently reverts to unscoped/unvalidated writes; couple the reconcile-sink to a required scope set (or pin the coupling) at the 1.0 surface freeze.~~ **RULED (spec 037-01, [ADR-0017](decisions/adr-0017-airlock-1-0-api-contract.md)):** `cookies.reconcile` stays HOST-INTERNAL and is explicitly **NOT FROZEN at 1.0** — no coupling is pinned into the frozen contract (pinning it would itself be a stability commitment on what is deliberately an internal sink, not a connector-facing grant). The fail-open-by-omission risk is named directly in `capability.d.ts`'s `reconcile` docstring as a named, unpinned integrator risk, not closed by a shipped guarantee; (iv) **arch #2 (035-01) value-side residual** — the scope is NAME-only; the cookie VALUE + non-stripped attributes (`path`/`expires`/`max-age`) are unvalidated on the reconcile path (`reconcileForBrokerJar` drops only domain/secure/samesite; browsers reject control chars in `document.cookie`), incl. the Overview-item-2 "newline in value" case — a candidate hardening follow-on.
**Resolved (slice [043-01](specs/043-cookie-pair-scan-accessor/slice-01-extract-getcookievalue.md), 2026-09-11) — item 5:** the pair-scan loop duplication is retired. 026-04 added the third byte-identical "read a cookie value by exact name" copy (`adapters/eds/index.js`'s `readCookieValue`), tripping the [extract-on-third-caller convention](conventions.md)'s rule of three. Extracted the shared exact-name accessor `getCookieValue(cookieString, name)` into the pure, import-free leaf `core/cookie-parse.js` (mirrors `core/cookie-scope.js`'s leaf shape) and repointed both exact-name copies (`readCookieValue` — now deleted — and `adapters/eds/cookies.js`'s mediated `get(name)` inner loop, reconciled to its `Promise<string|null>` contract via `?? null`). Behavior-preserving (full `npx vitest run` green, 101 files / 1548 tests; new `test/cookie-parse.test.js` witnesses the edges; no `readCookieValue` code references remain). The prefix-match (`findGaStreamCookie`) / first-pair / filter / scoping **variants stay open-coded** — genuinely different accessors, not lazy non-extractions (spec 043 §A2); no generic pair-iterator introduced (leanness).
...
### OQ13-b — the `_gcl_au` linker-cookie writer on a rewired page (named MVP8; resolves MVP9 — from the 044-01 frame-critique)
...
> Deferred during 032-01 (`boot(config)` + the composite handle). The config surface is deliberately **experimental /
> not-yet-frozen**; a later config-surface freeze (a follow-on ADR extending ADR-0017) is where any of these that
> survive get frozen. None blocked 032-01. _(2026-09-07, ADR-0018: "the 1.0 API pin" here was the stable-core pin; "1.0"
> itself is now the adoption bar.)_
...
**Ruled by [ADR-0017](decisions/adr-0017-airlock-1-0-api-contract.md) (spec 037-01):** the 1.0 pin does NOT fix this
with a per-connector read namespace — composite read-namespacing and `sampled` surfacing are explicitly **NOT
FROZEN at 1.0**, riding the same experimental carve-out as the standalone per-connector boot handles (which vary
by design). A per-connector read namespace (e.g. `getState({connector})`) remains a candidate future addition if
a caller needs it, but is not committed by this ADR.
...
- **exposure routing couples to the mutable `window.airlock` global.** ✅ RESOLVED by spec 034-03. `boot()` now
  ...
  handle (a local predicate). Flagged for 037's surface-freeze. **RESOLVED (spec 037-01, 2026-09-05):** kept OFF the
  installed handle — `accepts` is physically removed from `createComposite`'s returned object (`adapters/eds/index.js`);
  `boot()` rebinds the alloy exposure reporter's `compositeEmit.accepts` to a local predicate over the booted
  connectors instead (byte-for-byte the same `acceptsEvent`-over-`booted` logic `composite.accepts` used, so no
  behavior change), so the frozen `window.airlock` shape is exactly `{ push, pushCritical, setConsent, getState,
  flushNow, stats, dispose }` ([ADR-0017](decisions/adr-0017-airlock-1-0-api-contract.md)). Item (a) above (nulling
  the ref on `dispose` for the silent-drop residual) remains open, unrelated to this closure.
...
- **endpoint-ceiling BREADTH grounding (creds-gated live-Alloy).** `bootAlloy`'s ceiling is the grounded interact
  ...
  holds it. Trigger: a real-Alloy adopter needs the server-directed syncs, or the alloy real-site residual (an Adobe-stack site — the reference intuit-class site runs no Adobe Web SDK, so the MVP9 rewire never exercises alloy; ADR-0018).
...
### ~~Worker-mapped GET-egress connectors DROP their ring tail at teardown (no GET critical dispatcher)~~ — RESOLVED 2026-09-10 (spec 042)
