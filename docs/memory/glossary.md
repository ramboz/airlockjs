...
## fan-out re-map
The seal mechanism (ADR-0024, spec 045-03) letting a connector whose `handle` fans one event out to N held beacons re-map EACH held beacon to its own wire form on grant: each ready beacon carries a distinct `EgressRequest.remapKey`, which the seal preserves hold→flush and threads into `remap(event, consent, remapKey)`. 1:1 connectors set no key and stay byte-identical. First consumer: Floodlight (046-03 — ccm + activity). Colliding keys are a per-connector authoring footgun (distinct-by-construction); coordinated fan-outs (a non-derivable cross-beacon value) are out of scope and re-open the decision.

## matrix wire
A beacon wire shape where params ride the URL PATH as `;`-delimited RFC-3986 matrix segments (`ad.doubleclick.net/activity;src=…;type=…`), not a query string — the DC Floodlight `activity` form (spec 046-02). Built by `core/path-matrix.js` (`appendMatrixParam`/`joinMatrixUrl`, a pure import-free leaf, sibling of `core/query-params.js`); the 038 parity oracle parses it via `fieldsFromMatrixUrl`, not `URL.searchParams`. See also [[segment-anchored prefix match]].

## segment-anchored prefix match
The endpoint-ceiling granularity (ADR-0025, spec 046-02, extends ADR-0006) for [[matrix wire]] endpoints: a declared origin+path containing `;` opts into a prefix match (`destination === prefix || destination.startsWith(prefix + ';')`), admitting a per-request path cachebuster (DC's `num`/`ord`); every query-delimited endpoint keeps the exact set-membership match. Anchored at the `;` boundary so a src value-extension can't escape the declared prefix. Residual: it pins origin+`/activity;src` but not `type`/`cat`, admitting arbitrary trailing `;`-segments (no new-destination escape).

## native-tag suppressor
The vendor-neutral, config-driven, adopter-facing module (spec 049, ADR-0030) that lets an EDS adopter's OWNED page stop a [[tag-manager container]] from firing migrated vendors' tags, so airlock's off-thread equivalents can be measured against them WITHOUT a container-profile (Tealium) change — the MVP9 developer-provable page-side after-arm (ADR-0029). Lives in `adapters/eds/tag-suppressor.js` (host/DOM layer, NOT `core/` — it patches DOM globals). Installed BEFORE the container loads, it neutralizes matched vendors by URL/query matcher on two surfaces: (a) runtime `<script>` injection — the FULL DOM-insertion surface (`appendChild`/`insertBefore`/`replaceChild`/`append`/`prepend`/`insertAdjacentElement`), the TBT source (049-01); and (b) direct beacon transports — `<img>` src/setAttribute/srcset, `navigator.sendBeacon`, `XMLHttpRequest`, `window.fetch` (049-02). Carves out airlock's own arm egress via a URL allow-set that wins over an over-broad matcher, PLUS the [[transport-of-emission carve-out]]; emits a loud FLAT spec-028 diagnostic per suppression (naming the URL + transport). Ships as a served dist sibling (like `reserve-personalization.js`). GATING proof is the real-browser rig `rig/tag-suppressor.mjs` (network-0 per transport under the reference CSP: Trusted Types + strict-dynamic; 45/45 assertions). Kill criterion (ADR-0030): a runtime inlined into the container bundle, or fetched inside a Worker / via `importScripts`, escapes the patched surface and falls back to the profile-side after-arm. Consumed by the 050 reference-site trial (`?martech=airlock`). See [[rewire]].
