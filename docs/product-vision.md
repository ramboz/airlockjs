...
Product-level unknowns (architecture-level ones live in `architecture.md`): whether to commit to a vendor-neutral event schema in MVP1 or let it emerge from the GA4 mapping and generalize after MVP2 (leaning emergent/minimal); how far the inspector goes in MVP1 versus later; and the distribution channel for the EDS audience (git subtree, matching the aem-martech/aem-experimentation convention, versus npm).
