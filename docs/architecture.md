...
- `core/*.worker.js` — the connector runtime (chambers) that run inside a Web Worker: `chamber.worker.js` (GA4 MP), `pixel-chamber.worker.js`, `dom-chamber.worker.js`, `ga4-gtag-chamber.worker.js`, `helix-rum-chamber.worker.js`, `google-ads-chamber.worker.js` — plus `core/worker-dom/` (the worker-side DOM-compat mirror, ADR-0014).
