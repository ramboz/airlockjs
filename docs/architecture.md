...
- `core/*.worker.js` — the connector runtime (chambers) that run inside a Web Worker: `chamber.worker.js` (GA4 MP), `pixel-chamber.worker.js`, `dom-chamber.worker.js`, `ga4-gtag-chamber.worker.js`, `helix-rum-chamber.worker.js`, `google-ads-chamber.worker.js`, `floodlight-chamber.worker.js` — plus `core/worker-dom/` (the worker-side DOM-compat mirror, ADR-0014).
- `adapters/eds/` — EDS integration: the three-phase hooks, block-decoration instrumentation, in-house decisioning for the eager window. `adapters/eds/tag-suppressor.js` — the **vendor-generic native-tag suppressor** (spec 049 / [ADR-0030](decisions/adr-0030-native-tag-suppressor.md), MVP9): a config-driven, **adopter-facing** primitive (host/DOM-coupled → `adapters/eds/`, NOT `core/`) that an EDS site installs BEFORE its tag-manager container loads to page-side-suppress a migrated vendor's native tags — the developer-controlled MVP9 after-arm ([ADR-0029](decisions/adr-0029-mvp9-developer-side-after-arm.md)) that needs no Tealium-profile change. It patches the full DOM script-insertion surface (`appendChild`/`insertBefore`/`replaceChild`/`append`/`prepend`/`insertAdjacentElement`) to neutralize a matching runtime `<script>` **before it connects** (pre-fetch, so a blocked runtime never downloads/evaluates — the TBT win; proven network-0 + sentinel-absent under a Trusted-Types + `strict-dynamic` CSP in `rig/tag-suppressor.mjs`), matches by **URL/path/query** (not host — so it discriminates a `?id=` subset for partial migration), **carves out airlock's own arm egress** by an allow-set that wins over an over-broad matcher, and emits a **flat** spec-028-shaped per-suppression diagnostic. Vendor-neutral (declared matchers, no hardcoded vendor — machine-enforced by a source-text grep guard) and shipped as a served `dist` sibling so an adopter subtrees + imports it. 049-02 extends the same matchers to direct beacon transports.
- `connectors/ga4/` — GA4 connectors. `map.js` = the MVP1 Measurement-Protocol (server-side / trusted-secret) path (frozen surface, ADR-0017). `gtag.js` = the additive MVP7 gtag-protocol path (`createGa4GtagConnector`, spec 039 / ADR-0019): originally a pure mapper whose `handle()` returns the `/g/collect` GET `EgressRequest[]` — no `api_secret`, so GA4 parity is a same-protocol beacon diff (038 harness) — now a full `Connector` (`{manifest, init, handle}`), **live** since spec 041: hosted in an egress-confined `core/ga4-gtag-chamber.worker.js` (`withholdFetch`, pixel-class) and booted via `adapters/eds/index.js`'s `bootGa4Gtag` / declarative `type:"ga4-gtag"` config, carrying Consent-Mode `gcs`/`gcd` (039-02/05) + `_ga_<stream>` session-state (039-03) and coalescing same-context bursts into gtag's batched `/g/collect` POST (`coalesceGa4`, spec 040). Being a worker-mapped GET-egress connector, its unload/teardown ring tail is flushed as `/g/collect` GET beacons via a main-thread `requestMapper` (`createGa4GtagConnector(connectorConfig).handle`) wired into `core/egress.js`'s critical dispatcher (spec 042-01); pixel flushes its `/tr` ring tail the same way (`createPixelConnector(connectorConfig).handle`, 042-02), and the `workerMappedGetEgress` drop-gate is retired now that every worker-mapped connector supplies its own critical mapper (a POST `mapper` or a GET `requestMapper`). `cookies.js` holds the host-called identity+session cookie surface: `sourceGa4Ctx` (reads `_ga`/`_ga_<stream>` for `cid`/`sid`, consent-gated 017-02) and `writeGa4SessionState` (the `_ga_<stream>` read-modify-write session writer that reproduces gtag's GS2 state machine, `analytics_storage`-gated cookie-WRITE governance surface, 039-03 — first wired host-side by `bootGa4Gtag`, spec 041-02). `connectors/alloy/` — MVP2 wrapped-SDK connector. `connectors/pixel/` — the generic GET wire-protocol pixel connector (spec 026): one `createPixelConnector` interpreter (`connectors/pixel/connector.js`) driven by a declarative per-vendor config (`connectors/pixel/vendors/{meta,linkedin,bing}.js`), plus 026-04's worker-side identity/advanced-matching hashing (`connectors/pixel/advanced-matching.js`). `connectors/helix-rum/` — the RUM/CWV connector (spec 030) that replaces EDS's inline `sampleRUM` checkpoints for `top`/`error`/`cwv`. `connectors/google-ads/` — the Google Ads (AW) ad-conversion connector (spec 044, MVP8): `createGoogleAdsConnector` reproduces the AW page-load `ccm/collect` GET beacon off-thread, carrying Consent Mode v2 (`gcs`/`gcd`/`npa`) + a read-only host-sourced `_gcl_au`→`auid` (never minted — §A5/OQ13-b), `ad_storage`-purposed so the seal gates it (slice 044-02). **Chamber-hosted + bootable since spec 048-01** (mirroring ga4-gtag/041): an egress-confined `core/google-ads-chamber.worker.js` (`withholdFetch`) booted via `adapters/eds/index.js`'s `bootGoogleAds` / declarative `type:"google-ads"` config (`KNOWN_CONNECTOR_TYPES` + the pinned `googleAdsConnector` schema `$def`), opting into the seal's `holdOnDenied` grant-flush re-map (`createGoogleAdsRemap`, `consentDefault`-consistent), its unload ring tail flushed as a `ccm/collect` GET via a main-thread `requestMapper`. `connectors/floodlight/` — the Floodlight (DoubleClick / DC) ad-conversion connector (spec 046, MVP8): `createFloodlightConnector` reproduces DC's **two** page-load beacons off-thread — the query-delimited `www.google.com/ccm/collect` (`auid`, 046-01) and the `;`-delimited matrix-path `ad.doubleclick.net/activity;src=…` (Floodlight-native `src`/`type`/`cat` + `auiddc`, 046-02) — both carrying Consent Mode v2 (`gcs`/`gcd`/`npa`) + a read-only host-sourced `_gcl_au`-derived linker (never minted, §A4), `ad_storage`-purposed. Both opt into the seal's `holdOnDenied` **fan-out** re-map via a per-beacon `remapKey` + a key-aware `createFloodlightRemap` (046-03 / [ADR-0024](decisions/adr-0024-fanout-remap-per-beacon-key.md)); the activity form's matrix wire rides `core/path-matrix.js` (`appendMatrixParam`/`joinMatrixUrl`) + a segment-anchored endpoint-ceiling prefix match ([ADR-0025](decisions/adr-0025-endpoint-ceiling-matrix-prefix-match.md)). **Chamber-hosted + bootable since spec 048-02** (mirroring 048-01/google-ads): an egress-confined `core/floodlight-chamber.worker.js` booted via `bootFloodlight` / declarative `type:"floodlight"` config (+ the pinned `floodlightConnector` schema `$def`); being two-origin, its boot passes a **two-element** endpoint ceiling `[ccm, activity]` — the activity element via the shared `deriveActivityCeilingEndpoint` (one home for the matrix-join, so the manifest ceiling + boot ceiling can't drift). Two collisions with Floodlight's native `type` config field are resolved: the boot/config field is renamed **`activityType`** (the reserved connector-kind discriminant), and the chamber init message **nests** `connectorConfig` rather than spreading it (the `type:"init"` wire discriminant). `connectors/consent-mode.js` — the shared **gtag-family** Google Consent Mode v2 (`gcs`/`gcd`/`npa`) encoders, reused by `ga4-gtag` + `google-ads` + `floodlight` (`gcs`/`gcd` extracted 044-01, `npa` lifted from `google-ads` 046-01 on Floodlight's arrival; deliberately connector-side, NOT `core/`, because they emit Google-specific wire strings and `core/` carries no vendor coupling — the `docs/conventions.md` § Code home rule). Registry namespace `airlock/ga4`, `airlock/ga4-gtag`, `airlock/alloy`, `airlock/pixel/meta`, `airlock/pixel/linkedin`, `airlock/pixel/bing`, `airlock/helix-rum`, `airlock/google-ads`, `airlock/floodlight`.
- `drivers/consent/` — host-neutral **consent-input drivers**: the *source* side of the seal (produce a consent vector to feed IN), distinct from `core/consent.js`'s egress enforcement, onto ADR-0007's consent-input seam. `drivers/consent/onetrust.js` — the OneTrust consent-input driver (spec 047 / [ADR-0026](decisions/adr-0026-onetrust-consent-input-source.md)): a **main-thread, zero-import pure leaf** that reads OneTrust's **resolved-consent** surface (`OnetrustActiveGroups` / the `OptanonConsent` cookie flags — **never** `GetDomainData().Status`, which is configured-default) and maps the site's OneTrust groups → the `core/consent.js` purpose vector via a **host-provided `{ groupId: ConsentPurpose[] }` map**, feeding the `adapters/eds/index.js` `consent` boot param (`bootGa4Core`, guarded / back-compat). No chamber, no worker, no egress; the OneTrust read is dependency-injected (fixture-testable). It also subscribes to OneTrust's `OnConsentChanged` / `OptanonWrapper` change signals to call the runtime handle's `setConsent` on a mid-session change (spec 047-02) — the "OneTrust-accept flow" that flushes 045-held ad beacons on the grant edge. **Since spec 048-03 ([ADR-0027](decisions/adr-0027-onetrust-composite-governance-field.md)) `onetrust` is also a top-level `boot(config)` governance field** (a sibling of `consent`/`consentStrict`/`payloadDenylist`, pinned in `contracts/instrumentation-config.schema.json`): `boot(config)` derives the boot vector into `governance.consent` and wires ONE change-subscription onto the composite `setConsent` fan-out, so a mid-session accept flushes held Google Ads **and** Floodlight beacons across all booted connectors. The composite is the sole subscriber **by construction** (per-connector `onetrust` in a config entry is stripped; `config.onetrust` is never threaded to a sub-boot) — not via the driver's identity-keyed idempotency guard, which cannot express precedence (ADR-0027). **`subscribeOnetrustConsentChanges` returns an idempotent, re-boot-safe `unsubscribe()`** ([ADR-0028](decisions/adr-0028-onetrust-unsubscribe-reboot-safety.md)): each surface is wired once as a stable trampoline over a mutable active-handler slot, a re-subscription swaps the slot, and the compare-and-clear unsubscribe (folded into the handle's / composite's `dispose()`) lets a re-`boot()`'s new subscription take over without the prior teardown stranding it. Deliberately **not** `core/` (carries OneTrust-format-coupled reads) and **not** a `connectors/` (opens no chamber, emits no egress) — the `docs/conventions.md` § Code home rule.
...
- **The airlock (capability bridge)**: connectors request mediated capabilities rather than reaching for browser APIs. DOM injection is fulfilled by the orchestrator *through* `reserveSpace` / `insertAfterInteraction`, so injected content is CWV-safe by construction. Egress is fulfilled through the egress seam and held at the seal until consent/allowlist pass. The seal's verdict (`core/consent.js` `egressVerdict`, enforced in `core/airlock.js`) is **pending → hold** (buffer + flush-on-grant), **granted → send**, **strict → drop**; a connector's instance may additionally **opt into `holdOnDenied`** (ADR-0023 Option E, spec 045) so a *denied* governing purpose **holds** instead of sending — grounded per vendor, not blanket (g-ads holds; GA4 sends cookieless). A held beacon on such an instance is **re-mapped** under the now-current consent when flushed on grant — rebuilt from its source event via the connector's `remap` (carried on the additive optional `EgressRequest.event` channel), with the re-mapped URL re-checked against the endpoint ceiling — never a stale under-denial re-send. A connector whose `handle` fans one event out to **multiple** held beacons tags each with an additive optional `EgressRequest.remapKey`, threaded into `remap(event, consent, remapKey)` so each held beacon re-maps to its own wire form (ADR-0024, spec 045-03). Enforcement stays at the **core seal**; alloy (wrapped-SDK) is a separate path (`core/wrapped-sdk-host.js`).
...
3. **The connector interface** — what a connector implements: consume typed events → request capabilities → emit to a declared endpoint. A mapper MAY **throw** on contract-invalid input (e.g. GA4 `mapToMp` rejects a `purchase` missing `transaction_id`/`currency`/`value`/`items[]`, spec 008); the airlock isolates that throw to the failing **descriptor** per Q1 — a per-event catch in the generic connector host (`core/connector-host.js` `routeBatch`, spec 012-01, converged onto by GA4 in spec 014-03; formerly a GA4-hardcoded per-descriptor catch in `chamber.worker.js`'s retired `mapBatch`, spec 009-01) drops just that event and records it in the cycle's `dropped[]`, and the main thread surfaces each drop plus any chamber-level `worker.onerror` through an injectable diagnostics seam (spec 009-02). OQ14 **resolved**; the unload/critical fast path is not yet routed through `routeBatch` (**OQ16** — by design, spec 014-03: the synchronous unload path stays a separate, worker-independent path reusing the connector's own main-thread mapper directly — `mapToMp` for GA4-MP, `mapToRum` for helix-rum since 030-01, and the connector's `handle` via a GET `requestMapper` for ga4-gtag (042-01) and pixel (042-02) — `core/egress.js`). Note OQ16's own concern (a throwing mapper on the unload path is not per-event-isolated like `routeBatch`) is unchanged by 042-01: the GET `requestMapper` path is not wrapped in a per-event catch either, inert only because the current GET connectors' `handle` does not throw. **026-04 ([ADR-0022](decisions/adr-0022-pixel-advanced-matching-hashing.md)) adds a pixel-only side-channel to this same worker↔main boundary, alongside the `{ready, dropped}` cycle output**: a dedicated `{type:"identity", raw}` message (main→worker) feeds raw post-boot Meta advanced-matching PII (`em`/`ph`/…) to the pixel chamber (the boot `external_id` instead rides the `init` message's `advancedMatching` field, eager-hashed at boot) — bypassing `push()`/`payloadDenylist` by design, safe under the chamber's `withholdFetch` egress confinement — and the chamber posts back `{type:"identity", ud:{<field>:<hex>}}` (worker→main, SHA-256 hashes only, never the raw value). The chamber merges the resolved `ud[...]` onto every steady-state `/tr` beacon; `core/airlock.js` additionally holds a hash-only per-field identity cache so the spec-042 pixel unload `requestMapper` merges them onto the closing beacon synchronously; the returned handle gains an additive `setIdentity(raw)` verb (ADR-0017 permits additive, non-breaking extensions to the frozen adopter handle shape).
