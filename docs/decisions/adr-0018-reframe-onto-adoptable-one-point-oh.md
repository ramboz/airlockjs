...
| **MVP7** | **Pixel Parity & the Parity Harness** (was "Connector Breadth") | v0.7.0 | **risk-first: the GA4 half of the R-009 gtag-family fidelity spike** — can Measurement-Protocol egress reach console-level parity with a container's `gtag.js` (`/g/collect`) tag: the semantic field-map, session/engagement (the open OQ13-2 `_ga_<stream>` session gap is a *known* divergence on an MPA), Consent Mode state · the vendor-generic golden-replay **parity harness** (per-protocol oracles) · **Meta Pixel parity** — 026-04 un-deferred (identity/advanced-matching + POST), now groundable on redacted real captures · **GA4 parity** through the harness · breadth-as-configs + drop-in ergonomics become *variable* |
| **MVP8** (new) | **Ad-Conversion Offloading** | v0.8.0 | risk-first: **the AW/DC half of R-009** — conversion-ping fidelity (Consent Mode v2, `gclid`/`_gcl_au`, linker, `wbraid`/`gbraid`) · **Google Ads** + **Floodlight** connectors · the **OneTrust consent-input driver** (R-007 §4) · Segment → variable/later |
...
| Artifact | Disposition | Detail |
|---|---|---|
| `docs/releases/mvp6.md` | `rewrite` | Retitle **"MVP6 — Stable Core & Validation Harness"**; Include row 3 → "the **stable-core contract** (frozen surface + guards)"; release-check "a documented 1.0 stability commitment" → "a documented stable-core contract"; the Split row (full-stack hosting → R-007) → points at R-007 as **the 1.0 benchmark** owned by MVP7–9; status → `shipped` as **v0.6.0** when E1 lands, with a ship block that mirrors mvp5.md's **including its honest-caveats form**: **(a)** Include row 2's **018-h** (reserveSpace eager-phase production wiring) — recorded "still open" in `docs/refinement-todo.md:64` / spec 018 — is **de-facto closed by 033-03** (`reservePersonalization`, the synchronous eager pre-paint entrypoint called from `loadEager`, handed off to `boot(config)`; `adapters/eds/index.js` ~:1383–1415): annotate the row SHIPPED-via-033-03 and reconcile the ledger (refinement-todo row below); **(b)** release-check criterion 6 ("a real production site runs airlock's supported connectors with CWV preserved") is **NOT met** — the harness + procedure shipped (036), but the live run is the operator's creds-gated step and has not been run (`036 slice-01:100–101`): the ship block states this as a caveat and **carries the criterion to MVP9**, where the rewire runs it for real. v0.6.0 is cut on what shipped, never on a criterion claimed met. Historical rationale lines ("the API is pre-1.0 — adoption needs a stability contract") stay as the problem statement they were. |
...
- **E6 — R-009 research note — gtag-family fidelity**, in two halves: **(a) GA4** (MVP7 risk-first): the `/g/collect` → Measurement-Protocol semantic field-map, session/engagement continuity on an MPA (OQ13-2), Consent Mode state, and the console-level check — decides whether MP-only reaches parity or a second, *additive* gtag-protocol GA4 connector is needed; **(b) Google Ads + Floodlight** (MVP8 risk-first): conversion-ping fidelity — Consent Mode v2, `gclid`/`_gcl_au`, linker, `wbraid`/`gbraid`.
...
- **Assumed, not probed:** that Google Ads (`gtag` AW conversion pings) and Floodlight (DC activity pings) can be reproduced off-thread with attribution parity — including Consent Mode v2, `gclid`/`_gcl_au` capture and linker behaviour. This is exactly why MVP8 is spike-first (the AW/DC half of R-009); if the spike shows an unbridgeable gap, see Kill criteria.
...
- **Google Ads + Floodlight (MVP8 risk-first).** If the AW/DC half of R-009 shows conversion pings **cannot** reach attribution parity from a worker (e.g. Consent Mode / linker semantics that require the vendor runtime on the main thread), the 1.0 rewire set does **not** silently shrink to the non-dominant pair: those two tags carry most of the cited two-thirds of TBT, so "TBT-dominant tags rewired" would be false — the owner re-decides the bar (as in the next criterion), recorded by a follow-on ADR.
...
- GA4: MP-only parity vs an additive gtag-protocol connector — decided by the GA4 half of R-009 (MVP7 risk-first), per the first kill criterion.
